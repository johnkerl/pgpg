...
func usage() {
	...
	fmt.Fprintf(os.Stderr, "  -mode: int (default), float, mod.\n")
	fmt.Fprintf(os.Stderr, "  -mod: integer modulus for -mode=mod (required).\n")
...
func main() {
	...
	flag.StringVar(&mode, "mode", "int", "Arithmetic mode: int, float, mod")
	flag.IntVar(&modN, "mod", 0, "Integer modulus for -mode=mod")
	flag.Usage = usage
	flag.Parse()

	args := flag.Args()

	validModes := map[string]bool{"int": true, "float": true, "mod": true}
	if !validModes[mode] {
